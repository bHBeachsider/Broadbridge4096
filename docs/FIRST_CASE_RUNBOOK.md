...
The importer permits draft/reference-only records as **eval-only**, so import success alone is insufficient. The first live brief command enforces signed status, complete named/dated signoff, decided permitted use and at least one question. `undecided` cases are rejected by the importer. If any case is rejected the CLI exits 2; it may have written other admitted cases, but this runbook stops for review. Re-export into a fresh run directory after a correction.
