...
Bill Hurt enters cases directly in the Broadbridge Case Capture page, without a call first. The canonical contract is `broadbridge.case_record/1`. Bill is the named reviewer. Gate 0 requires recorded rights/storage and at least 30 Section C questions with reference answers across all five types, derived from signed cases. Pilot cases live in the Claude artifact store; originals/documents live in one approved S3 bucket.
