...
import { identitySchema, rightsSchema, candidateReviewSchema, hash, sourceCursorSchema, candidateCursorSchema, assertActionBudget, type ActionResult } from "../../lib/ingestion-validation";
...
async function protect<T>(work: (actor: string) => Promise<T>): Promise<ActionResult<T>> {
  try { return { ok: true, value: assertActionBudget(await work(await requireActor())) }; }
  catch (error) {
    if (typeof error === "object" && error !== null && "code" in error && error.code === "PREVIEW_TOO_LARGE") return { ok: false, error: "This complete preview exceeds the 1 MiB response budget. Review the full verified artifact offline; no evidence or candidate has been truncated or approved." };
...
export async function loadIngestionAction(cursor: unknown = null) { return protect(() => repository.listSources(sourceCursorSchema.parse(cursor))); }
export async function loadCandidatesAction(input: unknown, cursor: unknown = null) { return protect(() => service.candidatePage(identitySchema.parse(input), candidateCursorSchema.parse(cursor))); }
