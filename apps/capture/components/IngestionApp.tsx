...
import { uploadSchema, type Identity, type Snapshot, type SourceRow, type Preview, type Candidate, type ActionResult, type SourceCursor, type CandidateCursor, type CandidatePage } from "../lib/ingestion-validation";
...
function CandidateReview({ candidate, disabled, save }: { candidate: Candidate; disabled: boolean; save: (input: unknown) => Promise<void> }) {
  ...
  return <section className={styles.block}><h4>{candidate.example_id}</h4><p>Technical review: {candidate.status}</p><p className={styles.hash}>Candidate hash: {candidate.candidate_hash}</p><h5>Candidate messages and provenance</h5>{candidate.preview_oversized ? <p role="status">This complete candidate exceeds the preview response budget. Review the full verified artifact offline. Technical acceptance is disabled here; no candidate text has been truncated.</p> : <pre>{JSON.stringify(candidate.candidate_record, null, 2)}</pre>}{candidate.reason && <p>Recorded reason: {candidate.reason}</p>}<label htmlFor={`reason-${candidate.example_id}-${candidate.candidate_hash}`}>Technical review reason</label><textarea id={`reason-${candidate.example_id}-${candidate.candidate_hash}`} value={reason} onChange={e => setReason(e.target.value)} /><div className={styles.toolbar}>{(["approved", "rejected"] as const).map(decision => <button className="btn" disabled={disabled || !reason.trim() || candidate.preview_oversized} key={decision} onClick={() => void save({ example_id: candidate.example_id, candidate_hash: candidate.candidate_hash, expected_review_id: candidate.review_id, decision, reason })}>{decision === "approved" ? "Accept candidate" : "Reject candidate"}</button>)}</div></section>;
}
export default function IngestionApp({ initial, email }: { initial: Snapshot; email: string }) {
  ...
  const [sourceHistory, setSourceHistory] = useState<SourceCursor[]>([null]);
  const [candidateHistory, setCandidateHistory] = useState<CandidateCursor[]>([null]);
  ...
  async function refresh() {
    const result = await actions.loadIngestionAction(sourceHistory.at(-1) ?? null);
  ...
  async function perform(work: () => Promise<ActionResult<unknown>>, success: string) {
    ...
    try { const result = await work(); setNotice(result.ok ? success : result.error); if (result.ok) { await refresh(); if (selected) await loadCandidatePage(selected, candidateHistory); } }
    catch { setNotice("Connection interrupted. Refresh status before retrying."); } finally { setBusy(false); }
  }
  async function loadCandidatePage(source: SourceRow, history: CandidateCursor[]) {
    const result = await actions.loadCandidatesAction(identityOf(source), history.at(-1) ?? null);
    if (result.ok) { setCandidatePage(result.value); setCandidateHistory(history); } else setNotice(result.error);
  }
  async function changeCandidatePage(history: CandidateCursor[]) {
    if (!selected) return;
    setBusy(true);
    try { await loadCandidatePage(selected, history); } catch { setNotice("Candidate page unavailable. Retry."); } finally { setBusy(false); }
  }
  async function changeSourcePage(history: SourceCursor[]) {
    setBusy(true);
    try {
      const result = await actions.loadIngestionAction(history.at(-1) ?? null);
      if (result.ok) { setSnapshot(result.value); setSourceHistory(history); setSelected(null); setPreview(null); setCandidatePage(null); } else setNotice(result.error);
    } catch { setNotice("Source page unavailable. Retry."); } finally { setBusy(false); }
  }
  async function inspect(source: SourceRow) {
    setSelected(source); setPreview(null); setCandidatePage(null); setCandidateHistory([null]); setEvidenceError(""); setBasis(""); setPermittedUse(source.current_permitted_use); setBusy(true);
    try {
      const [result] = await Promise.all([actions.previewSourceAction(identityOf(source), source.job_id), loadCandidatePage(source, [null])]);
      if (result.ok) setPreview(result.value); else setEvidenceError(result.error);
    } catch { setEvidenceError("Preview unavailable. Refresh and retry."); } finally { setBusy(false); }
  }
  ...
  async function uploadItems() {
    ...
    try { for (const item of queue) {
      ...
      try {
        ...
        updateItem(item, completed.ok ? (completed.value.state === "queued" ? "Queued for CPU verification" : `Receipt verified; processing status: ${completed.value.state}`) : completed.error);
  ...
  return <main className={styles.page}><header className={styles.header}><div><h1>Source intake & review</h1><p>Broadbridge oil & gas · {email}</p></div><div className={styles.toolbar}><a href="/">Case capture</a><a href="/review/public-v1">Public-document scoring</a><label>Theme <select aria-label="Theme" value={theme} onChange={e => setTheme(e.target.value)}><option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option></select></label></div></header>
    ...
    <div className={styles.toolbar}><h2>Source register</h2><button className="btn" disabled={busy} onClick={() => void perform(() => actions.loadIngestionAction(sourceHistory.at(-1) ?? null), "Status refreshed.")}>Refresh status</button></div>
    <nav className={styles.toolbar} aria-label="Source pages"><button className="btn" disabled={busy || sourceHistory.length === 1} onClick={() => void changeSourcePage(sourceHistory.slice(0, -1))}>Previous sources</button><span>Source page {sourceHistory.length} · {snapshot.sources.length} shown{snapshot.next_cursor ? " · more available" : " · last page"}</span><button className="btn" disabled={busy || !snapshot.next_cursor} onClick={() => void changeSourcePage([...sourceHistory, snapshot.next_cursor])}>Next sources</button></nav><div className={styles.layout}><section className={styles.sources} aria-label="Source register">{!snapshot.sources.length && <p>No sources yet. Select files above to create pending source records.</p>}{snapshot.sources.map(source => <article key={`${source.source_id}/${source.revision_id}`} className={`${styles.source} ${selected?.source_id === source.source_id && selected.revision_id === source.revision_id ? styles.selected : ""}`}><h3>{source.original_filename}</h3><p className={styles.status}>{stateLabel(source)}</p><dl><dt>Source</dt><dd>{source.source_id}</dd><dt>Revision</dt><dd>{source.revision_id}</dd><dt>Format / size</dt><dd>{source.media_type} / {source.size_bytes.toLocaleString()} bytes</dd><dt>Rights</dt><dd>{source.current_permission_status} · {source.current_permitted_use}</dd><dt>Confidentiality</dt><dd>{source.confidentiality}</dd></dl>{source.error_code && <p role="status">Processing error: {source.error_code}</p>}<div className={styles.toolbar}><button className="btn" disabled={busy} onClick={() => void inspect(source)}>Review source</button>{!source.job_id && <button className="btn" disabled={busy} onClick={() => void perform(() => actions.completeUploadAction(identityOf(source)), "Receipt verified; processing status refreshed.")}>Verify and queue</button>}{source.job_id && ["retry", "failed"].includes(source.state ?? "") && <button className="btn" disabled={busy} onClick={() => void perform(() => actions.retryIngestionAction(identityOf(source), source.job_id), "Retry status checked. Exhausted jobs require operator attention.")}>Check retry</button>}</div></article>)}</section>
    <aside className={styles.review} aria-label="Source review">{!selected ? <><h2>Review the evidence</h2><p>Select a source to inspect extracted blocks, provenance, and candidate messages. Rights approval and technical review remain separate.</p></> : <><h2>{selected.original_filename}</h2><p className={styles.hash}>SHA-256: {selected.content_sha256}</p>{!selected.job_id && <label>Retry original upload<input type="file" disabled={busy} onChange={e => { const file = e.target.files?.[0]; if (file) setQueue([{ file, identity: identityOf(selected), status: "Ready to retry the pending revision" }]); }} /></label>}<h3>Source rights</h3><p>Current decision: {selected.current_permission_status}. This decision applies only to this source revision and hash.</p><fieldset disabled={busy}><label htmlFor="permitted-use">Permitted use</label><select id="permitted-use" value={permittedUse} onChange={e => setPermittedUse(e.target.value)}><option value="reference_only">Reference only</option><option value="testing_only">Testing only</option><option value="training">Training</option></select><label htmlFor="rights-basis">Rights basis / revocation reason</label><textarea id="rights-basis" value={basis} onChange={e => setBasis(e.target.value)} /><div className={styles.toolbar}>{(["approved", "revoked"] as const).map(decision => <button className="btn" key={decision} disabled={!basis.trim()} onClick={() => void perform(() => actions.reviewRightsAction({ ...identityOf(selected), decision, permitted_use: permittedUse, rights_basis: basis, expected_review_id: selected.current_rights_review_id }), "Rights decision recorded for this revision.")}>{decision === "approved" ? "Approve source rights" : "Revoke source rights"}</button>)}</div></fieldset>
      <h3>Extracted evidence & provenance</h3>{evidenceError ? <p role="status">{evidenceError}</p> : !preview ? <p>Open this source again to load its latest evidence.</p> : !preview.document ? <p>Extraction is pending or unavailable. No extracted evidence is available for review yet.</p> : <><p>Extraction disposition: {String(preview.document.status)}</p><details><summary>Parser, classification, quality flags, and attachments</summary><pre>{JSON.stringify({ parser: preview.document.parser, classification: preview.document.classification, labels: preview.document.labels, quality_flags: preview.document.quality_flags, attachments: preview.document.attachments }, null, 2)}</pre></details>{blocks.map((block, i) => <article key={i} className={styles.block}><strong>{String(block.block_id ?? `Block ${i + 1}`)} · {String(block.kind ?? "evidence")}</strong><pre>{JSON.stringify(block, null, 2)}</pre></article>)}</>}
      <h3>Technical candidate review</h3><p>Review candidate answers against their source evidence. Accepting a candidate does not grant source rights or release a dataset.</p><nav className={styles.toolbar} aria-label="Candidate pages"><button className="btn" disabled={busy || candidateHistory.length === 1} onClick={() => void changeCandidatePage(candidateHistory.slice(0, -1))}>Previous candidates</button><span>Candidate page {candidateHistory.length} · {candidatePage?.candidates.length ?? 0} shown{candidatePage?.next_cursor ? " · more available" : " · last page"}</span><button className="btn" disabled={busy || !candidatePage?.next_cursor} onClick={() => void changeCandidatePage([...candidateHistory, candidatePage?.next_cursor ?? null])}>Next candidates</button></nav>{!candidatePage && <p>Candidate messages are not loaded. Reopen this source to retry.</p>}{candidatePage && !candidatePage.candidates.length && <p>No candidate messages are available for this revision.</p>}{candidatePage?.candidates.map(candidate => <CandidateReview key={`${candidate.example_id}/${candidate.candidate_hash}`} candidate={candidate} disabled={busy} save={async input => { await perform(() => actions.reviewCandidateAction(input), "Technical review recorded."); }} />)}</>}</aside></div>
  </main>;
}
